...
def get_full_std_lib(indent=0):
    ...
    """
    code = '/*PyScript std lib. Autogenerated and licensed by BSD-2-clause.*/\n'
    code += get_partial_std_lib(FUNCTIONS.keys(), METHODS.keys(), indent)
    return code

def get_full_std_lib_module():
    code = '/*PyScript std lib. Autogenerated and licensed by BSD-2-clause.*/\n'
    code += 'var mod = {};\n'
    code += get_partial_std_lib(FUNCTIONS.keys(), METHODS.keys(), 0,
                                'mod.f_', 'mod.m_')
    code = code.replace(FUNCTION_PREFIX, 'mod.f_').replace(METHOD_PREFIX, 'mod.m_') # todo: bah!
    return JSModule('pyscript-std', code, imports=[], exports='mod')
...
class JSModule:
    ...
    def __init__(self, name, code, imports=None, exports=None):
        if not isinstance(name, str):
            raise ValueError('JSModule.name must be a string')
        if not isinstance(code, str):
            raise ValueError('JSModule.code must be a string')
        self._name = name
        self._code = code
    ...
    def imports(self, imports):
        ...
        if not isinstance(imports, (tuple, list)):
            raise ValueError('JSModule.imports must be a tuple/list')
        imports2 = []  # make a copy so its not changed from outside
        for imp in imports:
            if not isinstance(imp, str):
                 raise ValueError('Elements in JSModule.imports must be str')
            if ' as ' in imp:
                imports2.append(tuple(imp.split(' as ', 1)))
            else:
                imports2.append((imp, imp))
        self._imports = imports2
    ...
    def exports(self, exports):
        ...
        if isinstance(exports, str):
            ...
            exports = list(exports)  # make a copy so its not changed from outside
            for exp in exports:
                if not isinstance(exp, str):
                    raise ValueError('Elements in JSModule.exports must be str')
            self._exports = exports
        else:
            raise ValueError('JSModule.exports must be a str or tuple/list')
    
    # todo: this is an AMD module, maybe support UMD too, e.g. for node?
    def saves(self):
        deps = ', '.join([repr(imp[0]) for imp in self.imports])
        imports = ', '.join([imp[1] for imp in self.imports])
        if isinstance(self.exports, str):
            exports = self.exports;
        else:
            exports = ', '.join([exp for exp in self.exports])
            exports = '{' + exports + '}'
        code = 'define("%s", [%s], function (%s) {\n\n%s\n\nreturn %s;\n});'
        return code % (self.name, deps, imports, self.code, exports)
